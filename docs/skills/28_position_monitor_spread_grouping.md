...
- **Inner vs envelope shape ambiguity.** Two callers pass differently shaped dicts: agent.py passes the full state_history envelope (`{run_id, trade_plan: {...}, risk_verdict: {...}}`), Streamlit pre-unwraps to the inner plan (`{ticker, legs, ...}`). The `valid` check works in both shapes because `_extract_inner_plan` returns the inner regardless. The `risk_verdict.approved` check only fires when an envelope is provided (inner shape has no `risk_verdict` key, so the check is silently a no-op).
